...
That rule is where the Portals refusal came from: not caution, just an agent doing what the brief said, in a spot where a lazier setup would have quietly made something up.
...
Both corrections went into the design spec with a note explaining the change, not patched and forgotten. And the same verify-first habit ran down the whole stack: the lint gate for the spec was chosen by running two [Redocly](https://redocly.com) rulesets against a throwaway file and watching what failed, rather than trusting the docs' description of either.
...
The Boards reviewer got suspicious that some "list" endpoints returning a single object might secretly be wrapped arrays. It hand-checked nine endpoints against the raw Postman data. The implementation turned out correct in every case, including one endpoint that genuinely does wrap its result in an array, which the implementer had modeled differently from its eight flat siblings. A review that re-derives the answer is worth ten that read the summary.
...
The redo edited those fifteen lines as text and left the rest of the file alone. A diff ten times the size of your intention is your first warning that something you did not intend came along for the ride.
...
**Doc-prep** is the standing rule that no doc gets drafted from memory. Before writing, build the outline from a knowledge graph of the subject and fact-check every claim against a structural index of the code. For this migration the "doc" was an API contract, but the discipline is identical: the agents query the graph and the index directly, and that is what the fact-check stage runs against.
